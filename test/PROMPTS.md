...
```
请阅读 test/PROMPTS.md，然后执行边界条件测试。
```
...
【清理】
6. 运行：rm /tmp/test.txt
```

**测试目的**：验证特殊字符显示、敏感文件访问、危险命令拦截。

**覆盖边界**：代码块、反引号、引号嵌套、环境变量、Shell 元字符、通配符、反斜杠、Unicode、Emoji、敏感文件、危险命令。

**说明**：危险命令测试应当拒绝批准，仅验证拦截机制。
