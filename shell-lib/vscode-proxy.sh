#!/bin/bash
#
# vscode-proxy.sh - VSCode 本地代理客户端库
#
...
vscode_proxy_health() {
    ...
    if command -v curl &> /dev/null; then
        response=$(curl -s --noproxy "*" --connect-timeout "$_VSCODE_PROXY_TIMEOUT" "$url" 2>/dev/null)
...
vscode_proxy_open() {
    ...
    response=$(curl -s --noproxy "*" --connect-timeout "$_VSCODE_PROXY_TIMEOUT" "${url}?path=${encoded_path}" 2>/dev/null)
